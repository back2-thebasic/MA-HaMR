...
class ForwardKinematicsLayer(nn.Module):
    ...
    def __init__(self, args=None, parents=None, positions=None, device=None):
        ...
        if parents is None and positions is None:
            ...
            if not os.path.exists(smpl_fname):
                raise FileNotFoundError(f'SMPL file {os.path.abspath(smpl_fname)} does not exist. Please check the path., pwd: {os.getcwd()}')
            smpl_data = np.load(smpl_fname, encoding='latin1', allow_pickle=True)
