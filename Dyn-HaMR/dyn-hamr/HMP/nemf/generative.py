...
class Architecture(BaseModel):
    def __init__(self, args, ngpu, batch_num=0):
        ...
        smpl_fname = os.path.join(args.smpl.smpl_body_model, "MANO_RIGHT.pkl")
        smpl_data = np.load(smpl_fname, allow_pickle=True, encoding='latin1')
        ...
        print(os.getcwd(), '...', os.path.join(args.dataset_dir, f'mean-{args.data.gender}-{args.data.clip_length}-{args.data.fps}fps.pt'))
    ...
    def set_input(self, input):
        self.input_data = {k: v.float().to(self.device) for k, v in input.items() if k in [
            'pos', 'velocity', 'global_xform', 'angular', 'root_orient', 'root_vel', 'trans', 'contacts'
        ]}
