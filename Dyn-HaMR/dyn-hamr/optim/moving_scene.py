...
CONTACT_INDS = [MANO_JOINTS[jname] for jname in CONTACT_ORDERING]
CONTACT_THRESH = 0.5
...
class MovingSceneModel(BaseSceneModel):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        assert motion_prior.model_data_config in [
            "mano+joints",
            "mano+joints+contacts",
        ], "Only mano+joints motion prior configuration is supported!"
    ...
    def estimate_prior_velocities(self, trans, root_orient, body_pose, betas, data_fps):
        ...
        with torch.no_grad():
            ...
            mano_results = self.pred_mano(trans, root_orient, body_pose, betas)
    ...
    def apply_world2prior(self, trans, root_orient, body_pose, betas, inverse=False):
        ...
        cur_smpl_data = self.pred_mano(trans, root_orient, body_pose, betas)
    ...
    def infer_latent_motion(
        ...
        mano_results = self.pred_mano(trans, root_orient, body_pose, betas)
        joints = mano_results["joints3d"]  # (B, T, len(MANO_JOINTS), 3)
    ...
    def rollout_mano_steps(self, num_steps=-1):
    ...
    def rollout_latent_motion(
        ...
        smpl_results = self.pred_mano(trans, root_orient, body_pose, betas)
        joints = smpl_results["joints3d"]  # (B, T, len(MANO_JOINTS), 3)
        ...
        if self.motion_prior.model_data_config == "smpl+joints+contacts":
            ...
            # expand to full body
            full_contact_conf = torch.zeros((B, Tm1, len(MANO_JOINTS))).to(contact_conf)
            full_contact_conf[:, :, CONTACT_INDS] = (
                full_contact_conf[:, :, CONTACT_INDS] + contact_conf
            )
            full_contacts = torch.zeros((B, Tm1, len(MANO_JOINTS))).to(pred_contacts)
...
def estimate_velocities(trans, root_orient, joints3d, data_fps):
    ...
    - joints3d (B, T, len(MANO_JOINTS), 3) joints3d of SMPL prediction
    """
    B, T, _ = trans.size()
    h = 1.0 / data_fps
