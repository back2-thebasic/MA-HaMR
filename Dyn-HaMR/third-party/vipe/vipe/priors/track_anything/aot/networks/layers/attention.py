...
class MultiheadLocalAttentionV1(nn.Module):
    ...
    def forward(self, q, k, v):
        ...
        ) + self.relative_emb_v.unsqueeze(0).unsqueeze(-1)

        relative_emb = relative_emb.view(n, self.num_head, self.window_size * self.window_size, h * w)
        unfolded_k_mask = (
            self.pad_and_unfold(memory_mask)
            .bool()
            .view(1, 1, self.window_size * self.window_size, h * w)
            .expand(n, self.num_head, -1, -1)
        )

        if self.enable_corr:
            qk = (
                self.correlation_sampler(q, k).view(n, self.num_head, self.window_size * self.window_size, h * w)
                + relative_emb
            )
        else:
        ...
        output = (local_attn.unsqueeze(2) * unfolded_vu).sum(dim=3).permute(3, 0, 1, 2).view(h * w, n, c)
...
class MultiheadLocalAttentionV2(nn.Module):
    ...
    def forward(self, q, k, v):
        ...
        if self.qk_mask is not None and (h, w) == self.last_size_2d:
            ...
            memory_mask = torch.ones((1, 1, h, w), device=v.device).float()
            unfolded_k_mask = self.pad_and_unfold(memory_mask).view(1, 1, self.window_size * self.window_size, h * w)
            qk_mask = 1 - unfolded_k_mask
    ...
    def local2global(self, local_attn, height, width):
        ...
        global_attn[local_mask.expand(batch_size, self.num_head, -1, -1, -1)] = local_attn.transpose(-1, -2).reshape(-1)
...
class MultiheadLocalAttentionV3(nn.Module):
    ...
    def forward(self, q, k, v):
        ...
        padded_qk[padded_local_mask.expand(n, self.num_head, -1, -1, -1)] += relative_emb.transpose(-1, -2).reshape(-1)
        padded_qk = self.dropout(padded_qk)

        local_qk = padded_qk[padded_local_mask.expand(n, self.num_head, -1, -1, -1)]

        global_qk = padded_qk[:, :, :, self.max_dis : -self.max_dis, self.max_dis : -self.max_dis].reshape(
            n, self.num_head, h * w, h * w
        )

        local_attn = torch.softmax(
            local_qk.reshape(n, self.num_head, h * w, self.window_size * self.window_size),
            dim=3,
        )
        global_attn = torch.softmax(global_qk, dim=3)

        agg_bias = torch.einsum("bhnw,hcw->nbhc", local_attn, self.relative_emb_v).reshape(h * w, n, c)
    ...
    def compute_mask(self, height, width, device=None):
        ...
        if self.padded_local_mask is not None and (height, width) == self.last_size_2d:
            ...
            local_mask = padded_local_mask[:, :, :, self.max_dis : -self.max_dis, self.max_dis : -self.max_dis]
            pad_pixel = self.max_dis * self.dilation
            local_mask = F.pad(
                local_mask.float(),
                (pad_pixel, pad_pixel, pad_pixel, pad_pixel),
        ...
        if self.use_linear:
            ...
            def cat(X1, X2):
                if num_head > 1:
                    ...
                    X = torch.cat([X1, X2], dim=-1).view(-1, bs, num_head * hidden_dim)
                else:
...
class LocalGatedPropagation(nn.Module):
    ...
    def forward(self, q, k, v, u, size_2d):
        ...
        if self.use_linear:
            ...
            if self.num_head > 1:
                v = v.view(-1, 2, self.num_head, hidden_dim // 2, h * w).permute(0, 2, 1, 3, 4).reshape(n, -1, h, w)
                u = u.view(-1, 2, self.num_head, hidden_dim // 2, h * w).permute(4, 0, 2, 1, 3).reshape(h * w, n, -1)
            else:
        ...
        if self.enable_corr:
            ...
            unfolded_k = self.pad_and_unfold(k).view(
                n * self.num_head, self.d_att, self.window_size * self.window_size, h, w
            )
            qk = (
                (q.unsqueeze(2) * unfolded_k)
                .sum(dim=1)
