...
class LongShortTermTransformer(nn.Module):
    def __init__(
        ...
        if final_norm:
            num_norms += 1
        self.decoder_norms = [_get_norm(d_model, type="ln") for _ in range(num_norms)] if num_norms > 0 else None
...
class DualBranchGPM(nn.Module):
    def __init__(
        ...
        if final_norm:
            num_norms += 1
        self.decoder_norms = (
            [_get_norm(d_model * 2, type="gn", groups=2) for _ in range(num_norms)] if num_norms > 0 else None
        )
...
class LongShortTermTransformerBlock(nn.Module):
    def __init__(
        ...
        self.long_term_attn = MultiheadAttention(d_model, att_nhead, use_linear=False, dropout=lt_dropout)
...
class LongShortTermTransformerBlockV2(nn.Module):
    ...
    def fuse_key_value_id(self, key, value, id_emb):
        ...
        bs = key.size(1)
        K = key.view(-1, bs, self.att_nhead, self.d_model // self.att_nhead) * (1 + torch.tanh(ID_K)).unsqueeze(-1)
...
class GatedPropagationModule(nn.Module):
    def __init__(
        ...
        self.self_attn = GatedPropagation(d_model * 2, d_model * 2, self_nhead, d_att=d_att)
    ...
    def forward(
        ...
        curr_QV = self.linear_QV(_tgt)
        curr_QV = torch.split(curr_QV, [self.d_att * self.att_nhead, self.expand_d_model], dim=2)
        curr_Q = curr_K = curr_QV[0]
        local_Q = seq_to_2d(curr_Q, size_2d)
        ...
        if tgt_id is None:
            tgt_id = 0
            cat_curr_U = torch.cat([silu(curr_U), torch.ones_like(curr_U)], dim=-1)
            curr_ID_V = None
        else:
        ...
        if curr_id_emb is not None:
            ...
            _, global_ID_V = self.fuse_key_value_id(None, curr_ID_V, curr_id_emb)
        ...
        cat_tgt2, _ = self.long_term_attn(curr_Q, global_K, cat_global_V, cat_curr_U, size_2d)
        cat_tgt3, _ = self.short_term_attn(local_Q, local_K, cat_local_V, cat_curr_U, size_2d)
