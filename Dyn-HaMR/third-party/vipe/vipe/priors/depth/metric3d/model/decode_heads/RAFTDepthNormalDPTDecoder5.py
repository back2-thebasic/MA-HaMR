...
class ConvLoRA(nn.Conv2d, LoRALayer):
    def __init__(
        self, in_channels, out_channels, kernel_size, r=0, lora_alpha=1, lora_dropout=0.0, merge_weights=True, **kwargs
    ):
...
class Token2Feature(nn.Module):
    ...
    def __init__(
        ...
        self.scale_factor = scale_factor
        self.readoper = Readout(
            in_features=vit_channel,
            use_cls_token=use_cls_token,
            num_register_tokens=num_register_tokens,
            tuning_mode=tuning_mode,
        )
        if scale_factor > 1 and isinstance(scale_factor, int):
            self.sample = ConvTranspose2dLoRA(
                r=8 if tuning_mode == "lora" else 0,
...
class DecoderFeature(nn.Module):
    def __init__(
        self,
        vit_channel,
        num_ch_dec=[128, 256, 512, 1024, 1024],
        use_cls_token=True,
        tuning_mode=None,
    ):
...
class RAFTDepthNormalDPT5(nn.Module):
    def __init__(self, cfg):
        ...
        self.decoder_channels = (
            cfg.model.decode_head.decoder_channels
        )  # [128, 256, 512, 1024, 1024] [-, 1/4, 1/7, 1/14, 1/14]
        ...
        self.token2feature = EncoderFeature(
            self.in_channels[0],
        ...
        self.normal_predictor = nn.Sequential(
            ...
            # nn.BatchNorm2d(128),
            nn.ReLU(inplace=True),
            Conv2dLoRA(128, 128, kernel_size=1, r=8 if tuning_mode == "lora" else 0),
            nn.ReLU(inplace=True),
            Conv2dLoRA(128, 128, kernel_size=1, r=8 if tuning_mode == "lora" else 0),
            nn.ReLU(inplace=True),
        ...
        self.context_feature_encoder = ContextFeatureEncoder(
            self.feature_channels,
            [self.hidden_dims, self.context_dims],
            tuning_mode=tuning_mode,
        )
        ...
        self.update_block = BasicMultiUpdateBlock(
            cfg, hidden_dims=self.hidden_dims, out_dims=6, tuning_mode=tuning_mode
        )
    ...
    def forward(self, vit_features, **kwargs):
        ...
        if self.use_cls_token == True:
            vit_features = [
                [
                    ...
                    ft[:, 0 : 1 + num_register_tokens, :].view(
                        B, 1, 1, self.in_channels[0] * (1 + num_register_tokens)
                    ),
                ]
        ...
        if self.training:
            low_resolution_init = [
                self.clamp(depth_init[:, :1] * self.regress_scale + self.max_val),
                depth_init[:, 1:2],
                norm_normalize(depth_init[:, 2:].clone()),
            ]
